...
local function tasklist_label(c, args)
   ...
   if capi.client.focus == c then
    ...
    text = text .. "</span>"
    return text, bg, bg_image, not tasklist_disable_icon and c.icon or nil, is_focused
end
...
local function my_update_function(w, buttons, label, data, objects)
    ...
    for i, o in pairs(objects) do
        ...
        local text, bg, bg_image, icon, is_focused = label(o)
...
function tasklist.new(screen, filter, buttons, style, update_function, base_widget)
    local uf = update_function or my_update_function
